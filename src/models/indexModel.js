...
function createModelName() {
  ...
  for (const modelNameKey in models) {
    ...
    if (!modelNameRegex.test(modelNameKey) || !exist) {
      ...
      log(
        `Error: You need to check --> '${modelNameKey}' <-- which is static set.( 'filename + Model' like as 'userModel' )`
      );
    }
    if (typeof models[modelNameKey] !== "function" && exist) {
      delete models[modelNameKey];
      log(
        `Error: ${modelNameKey} from statically set, Has not set proper mongoose.model function, Check in ${__dirname}/${modelNameKey.replace(
          "Model",
          ".js"
        )}`
      );
  ...
  const modelFiles = fs
    ...
    .filter(
      (file) =>
        file !== "indexModel.js" &&
        !setToStatic.includes(file.replace(".js", ""))
    );
  for (const file of modelFiles) {
    ...
    if (modelNameRegex.test(modelNameKey)) {
      ...
      if (
        !checkObjectHasOneModel.includes("$caught") &&
        checkObjectHasOneModel.length < 17
      )
        log(
          `Error: More then one model exported in ${__dirname}/'${modelNameKey.replace(
            "Model",
            ".js"
          )}' model file.`
        );
        ...
        log(
          `Error: ${modelNameKey}, Has not set proper mongoose.model function, Check in ${__dirname}/${modelNameKey.replace(
            "Model",
            ".js"
          )}`
        );
      ...
      log(
        `Error: You have to rename '${modelNameKey.replace(
          "Model",
          ".js"
        )}' file, thisIsDromedaryCamelCase formate used, Special characters and symbols are not allowed including white space.`
      );
    }
  }
  ...
  if (env === "development" || env === "local") {
    ...
    console.log(
      `----------------------- Number of Models : ${
        setToStatic.length + modelFiles.length
      } -----------------------`
    );
    console.log(
      `----------------------- Available Models : ${
