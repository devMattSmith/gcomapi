...
} = require("../lib/responseMessages/message");
const {
  find,
  counts,
  findOne,
...
module.exports = {
  ...
  create: async (req, res, next) => {
    try {
      ...
      if (!isValidEmail) {
        return next(
          Boom.notFound((await responseMessageObject("email")).invalidProp)
        );
      }
      ...
      if (isUserExist)
        return next(
          Boom.notFound((await responseMessageObject("User")).alreadyExistError)
        );
      ...
      if (!user)
        return next(
          Boom.notFound((await responseMessageObject("User", "created")).error)
        );
      const verification = await createToken(user);
      const payload = await update({ _id: user._id }, { verification });
      return res.status(200).json({
        message: (await responseMessageObject("User", "created")).success,
        payload,
      });
    } catch (error) {
      return next(Boom.notAcceptable(error.message));
  ...
  //       if (!existingUser)
  //         return next(
  //           Boom.notFound((await responseMessageObject("User")).notExistError)
  //         );

  //       if (params.newPassword) {
  //         const isMatchNewPassword = await compareHashPassword(
  //           params.newPassword,
  //           existingUser.password
  //         );
  //         if (isMatchNewPassword)
  //           throw staticResponseMessageObject.duplicatePassword;
  ...
  //           $and: [{ email: params.email }, { _id: { $ne: id } }],
  //         });
  //         if (isUserExist)
  //           return next(
  //             Boom.notFound(
  //               (await responseMessageObject("User")).alreadyExistError
  //             )
  //           );
  ...
  //       if (!user)
  //         return next(
  //           Boom.notFound((await responseMessageObject("User", "updated")).error)
  //         );

  //       return res
  //         .status(200)
  //         .json({
  //           message: (await responseMessageObject("User", "updated")).success,
  //           user,
  //         });
  //     } catch (error) {
  //       log(error);
  ...
  //       if (!user)
  //         return next(
  //           Boom.notFound((await responseMessageObject("User", "deleted")).error)
  //         );

  //       return res
  //         .status(200)
  //         .json({
  //           message: (await responseMessageObject("User", "deleted")).success,
  //         });
  //     } catch (error) {
  //       log(error);
  //       return next(Boom.notAcceptable(error.message));
