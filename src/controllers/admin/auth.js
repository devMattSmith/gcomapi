  ...
  login: async (req, res, next) => {
    try {
      ...
      if (!user)
        return next(
          Boom.notFound(
            (await responseMessageObject("credentials")).invalidProp
          )
        );

      if (user.verification && !user.verification.isVerified)
        throw staticResponseMessageObject.emailNotVerified;
      ...
      return res.status(200).json({
        message: (await responseMessageObject("User", "login")).success,
        payload,
      });
    } catch (error) {
      log(error);
